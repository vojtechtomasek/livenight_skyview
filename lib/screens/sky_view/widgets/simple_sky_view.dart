...
class _SimpleSkyViewState extends State<SimpleSkyView> {
  ...
  double? _lastHorizontalRotation;
  double? _lastVerticalRotation;
  ...
  List<StarDisplay> _getStarsData() {
    ...
    if (skyViewProvider.selectedStar != null &&
        (_cachedStars == null ||
            !_cachedStars!
                .any((s) => s.hip == skyViewProvider.selectedStar!.hip))) {
      _cachedStars = null;
    }
  ...
  List<ConstellationLine> _getConstellationData() {
    ...
    if (_lastConstellationType != null &&
        _lastConstellationType != provider.constellationType) {
      _cachedConstellations = null;
    }
    _lastConstellationType = provider.constellationType;
    
    // Clear cache if rotation has changed (this is the key fix!)
    if (_lastHorizontalRotation != null &&
        (_lastHorizontalRotation != provider.horizontalRotation ||
            _lastVerticalRotation != provider.verticalRotation)) {
      _cachedConstellations = null;
    }
    _lastHorizontalRotation = provider.horizontalRotation;
    _lastVerticalRotation = provider.verticalRotation;

    // Use the same time-based cache logic as stars
    final now = DateTime.now();
